...
def finalize_function_results(
    ...
    meta = {
        ...
        "evaluation_order": "pointcloudwise_with_target_once_per_object",
        "calibration_note": (
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "with one point-cloud-counted progress bar."
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Maximum visualized point clouds; 0 means all point clouds.",
    )
    parser.add_argument(
    ...
    pointcloud_count = sum(
        len(function_by_object.get(object_id, []))
        for object_id in object_ids
    )

    progress = tqdm(
        total=pointcloud_count,
        desc="目标识别与功能理解联合评测",
        unit="个点云",
        dynamic_ncols=True,
        colour="magenta",
        ...
        for object_id in object_ids:
            ...
            for instance_id, rows in function_by_object.get(
                ...
                for row in rows:
                    ...
                    if should_visualize_pointcloud:
