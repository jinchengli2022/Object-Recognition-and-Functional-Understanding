...
def evaluate(gt_manifest: Path, pred_manifest: Path, out_dir: Path, class_field: str) -> dict[str, object]:
    root = gt_manifest.resolve().parent
    gt_rows = read_tsv(gt_manifest)
    if gt_rows and class_field not in gt_rows[0]:
    ...
    for gt in gt_rows:
        ...
        for pred in candidates:
            ...
            if value > best_iou:
                best_iou = value
                best = pred
            if pred.get("pred_category_id") == gt[class_field]:
        ...
        if best is None:
            ...
            class_correct = pred_category == gt[class_field]
            category_correct += int(class_correct)

        by_category.setdefault(gt[class_field], []).append(class_match_iou)
        result_rows.append(
            {
                ...
                "gt_normal_class_39_id": gt.get("normal_class_39_id", ""),
                "gt_eval_class_id": gt[class_field],
...
def main() -> int:
    ...
    parser.add_argument(
        "--gt-manifest",
        type=Path,
        default=PACKAGE_ROOT / "dataset" / "target_recognition_manifest.tsv",
    )
    parser.add_argument("--pred-manifest", type=Path, required=True)
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=PACKAGE_ROOT / "results" / "target_recognition" / "evaluation",
    )
    parser.add_argument("--class-field", default="category_id", help="GT manifest field used for class matching, e.g. category_id or normal_class_39_id.")
    args = parser.parse_args()

    summary = evaluate(args.gt_manifest, args.pred_manifest, args.out_dir, args.class_field)
