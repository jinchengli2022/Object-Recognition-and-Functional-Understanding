...
class BasicPointCloud_torch(NamedTuple):
    points: torch.tensor
    colors: torch.tensor
...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float = 1):
        ...
        scales = 1.1*torch.log(torch.sqrt(dist2))[...,None].repeat(1, 3)
    ...
    def create_from_pcd_torch(self, pcd : BasicPointCloud_torch, spatial_lr_scale : float = 1):
        self.spatial_lr_scale = spatial_lr_scale
        ...
        dist2 = torch.clamp_min(distCUDA2(pcd.points.float().cuda()), 0.0000001)
        scales = 1.1*torch.log(torch.sqrt(dist2))[...,None].repeat(1, 3)#smaller scale, larger gaussian size
        rots = torch.zeros((fused_point_cloud.shape[0], 4), device="cuda")
        rots[:, 0] = 1

        opacities = inverse_sigmoid(0.8 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda"))#smaller opacity, higher transparent
        language_feature = pcd.language_feature.float().cuda()
        # torch.zeros((fused_point_cloud.shape[0], 512), device="cuda")

        self._xyz = nn.Parameter(fused_point_cloud.requires_grad_(True))
        self._features_dc = nn.Parameter(features[:,:,0:1].transpose(1, 2).contiguous().requires_grad_(True))#color
        self._features_rest = nn.Parameter(features[:,:,1:].transpose(1, 2).contiguous().requires_grad_(True))
        self._language_feature = language_feature.requires_grad_(True)
        self._scaling = nn.Parameter(scales.requires_grad_(True))
        self._rotation = nn.Parameter(rots.requires_grad_(True))
        self._opacity = nn.Parameter(opacities.requires_grad_(True))
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
...
class Renderer:
    ...
    This class converts 3D point clouds (represented as Gaussian ellipsoids) into
    2D rendered images using differentiable rasterization.

    It supports multiple initialization modes (random, from point cloud, from file),
    and outputs not only RGB/depth images but also rendered per-point language
    features for cross-modal alignment.

    Args:
        sh_degree (int): Spherical harmonics degree for color representation.
        white_background (bool): Whether to use white (True) or black (False) background.
        radius (float): Default sampling radius for point initialization.

    Key Attributes:
        gaussians (GaussianModel): Core Gaussian parameter storage and interface.
        bg_color (torch.Tensor): Background RGB color tensor.
    """
    ...
    def initialize(self, input=None, num_pts=5000, radius=0.5):
        ...
        """
        Initialize the Gaussian model from different data sources.

        Args:
            input: Optional source for initialization.
                - None: Generate a random spherical point cloud
                - BasicPointCloud: CPU numpy-based object
                - BasicPointCloud_torch: PyTorch tensor-based object
                - str (path): Load point cloud from file
                - Any other type: Load from saved .ply
            num_pts (int): Number of points to sample if input=None
            radius (float): Sampling radius for random initialization
        """
        ...
        if input is None:
        ...
        elif isinstance(input, BasicPointCloud_torch):
            self.gaussians.create_from_pcd_torch(input, 1)
            ...
            self.gaussians._language_feature.requires_grad_(True)
        elif isinstance(input, str):
            xyz = extract_point_file(input)
    ...
    def render(
        ...
        """
        Render a Gaussian scene from a given camera viewpoint.

        Args:
            viewpoint_camera: Camera object with intrinsic/extrinsic matrices.
            scaling_modifier (float): Scale multiplier for Gaussian size.
            bg_color (torch.Tensor, optional): Custom background RGB color.
            override_color (torch.Tensor, optional): Replace Gaussian colors with custom ones.
            compute_cov3D_python (bool): If True, compute 3D covariance explicitly.
            convert_SHs_python (bool): If True, convert spherical harmonics to RGB in Python.

        Returns:
            dict: {
                "image": rendered RGB tensor [C, H, W],
                "depth": depth map [1, H, W],
                "alpha": alpha transparency map,
                "viewspace_points": screen-space Gaussian centers,
                "visibility_filter": mask of visible points,
                "radii": Gaussian screen-space radii,
                "rendered_idx": indices of rendered Gaussians,
                "rendered_contrib": per-pixel contribution weights,
                "language_feature_image": projected feature map [C, H, W],
            }
        """
        ...
        language_feature_precomp = self.gaussians.get_language_feature

        # Rasterize visible Gaussians to image, obtain their radii (on screen).
        rendered_image, radii, rendered_depth, rendered_alpha, rendered_idx, rendered_contrib, language_feature_image = rasterizer(
            means3D=means3D,
            means2D=means2D,
            shs=shs,
        ...
        return {
            ...
            "rendered_contrib": rendered_contrib,
