...
class TransformWrapper:
    ...
    def __call__(self, sample):
        pc, label = sample['pc'], sample['label']
        pc, label = self.transform(pc, label)
        return {'pc': pc, 'label': label}
...
class PointcloudJitter(object):
    def __init__(self, std=0.005, clip=0.05):
    ...
    def __call__(self, pc, label):
        ...
        for i in range(bsize):
            ...
            if len(drop_idx) > 0:
                ...
                pc[i, :, :] = cur_pc
                label[i, drop_idx] = label[i, 0].clone()
